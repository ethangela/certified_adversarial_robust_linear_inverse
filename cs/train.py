...
for epoch_i in range(start_epoch + 1, end_epoch + 1):
    ...
    for idx, data in enumerate(rand_loader):
        ...
        if smooth: #Smt-Grad 
            times = robust_noise_step
            in_times = robust_sample
            for j in range(1,times+1):
                optimizer.zero_grad()

                for k in range(in_times):
                    batch_n = torch.normal(0, ( (robust_noise/255.) / robust_noise_step )*j, size=batch_x.shape).type(torch.FloatTensor).to(device) #(1, 1, 33, 33)
        ...
        else: #Ord, Adv and Smt-Adv
            if attack: #Adv and Smt-Adv
                adv_img = batch_x.clone().detach().type(torch.FloatTensor).to(device).requires_grad_() 
                opt_x = torch.optim.SGD([adv_img], lr=alpha) 
                for i in range(attack_iter):
                    if sample: #Smt-Adv
                        for j in range(sample):
                            adv_img_noisy = torch.clamp(adv_img + std / 255. * torch.randn_like(adv_img).type(torch.FloatTensor).to(device), 0.0, 1.0)
                            x_input_x = [adv_img_noisy, get_cond(cs_ratio, 0.0, 'org_ratio')]
                            if j==0:
                                x_output = model(x_input_x, cur_Phi, Qinit, n_input)
                            else:
                        ...
                        x_input_x = [adv_img, get_cond(cs_ratio, 0.0, 'org_ratio')]
                        x_output = model(x_input_x, cur_Phi, Qinit, n_input)
