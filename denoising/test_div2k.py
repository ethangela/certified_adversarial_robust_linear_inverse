...
for img_idx in range(num_data): 
    ...
    #data
    img = Image.open(f'./data/DIV2K_valid/0{801+img_idx}.png').convert('L')
    img = img_crop(img, cropsize)
    if visual:
        model_dir = f'./DIV2K_output/{img_idx+1}'
    ...
    for i in range(test_attack_iter):
        ...
        optimizer.step() #adv_img.data = adv_img.data + test_alpha * adv_img.grad
        with torch.no_grad():
            ...
            # print(f'PGA info: diff_norm:{norm}. epsilon:{test_epsilon}. norm/eps:(>1? 1=no touch bound) {div}')
    ...
    with torch.no_grad():
        ...
        Ndis, Npsnr = PSNR(np.array(Noutput)*255., np.array(Noutput)*255., label) #newdis20230429
        if visual:
        ...
        if test_smooth:
            ...
            for j in range(test_sample):
                # ptb_img_noise, debug_noise = add_noise(ptb_img, test_std, device, debug=True) #A(x+\xi)+b+\delta #TODO
                ptb_img_noise = add_noise(ptb_img, test_std, device) #A(x+\xi)+b+\delta #TODO
